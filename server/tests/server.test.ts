  ...
  it("should update the todo2", (done) => {
    ...
    request(app)
      ...
      .expect((res) => {
        ...
        expect(typeof res.body.todo2.completedAt).toBe("string");
        //...
      })
      .end(done);
    // .end((err, res) =>{
      ...
      .end((err, res) => {
        ...
        Todo.findById(id)
          .then((todo2) => {
            ...
            expect(res.text).not.toBe(oriText);
            expect(res.completed).not.toBe(oriStatus);
...
describe("POST users", () => {
  ...
  it("should return validation errors if request invalid", (done) => {
    request(app)
      .post("/users")
      .send({ email: "wwwfrs.pl", password: "bbb" })
      .expect(400)
      .end(done);
  });
